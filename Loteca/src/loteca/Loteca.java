 ...
 * @author INSERIR AQUI OS NOMES DOS COMPONENTES DO GRUPO com seus Registros de Matrícula (um por linha) 
...
public class Loteca {
  ...
  * Funcionalidade: carrega todos os dados das Apostadors pré-cadastradas no banco de dados
  *               da empresa de “Recursos Humanos e Relacionamentos Cruzeiro do Sul S.A”.
  */
  ...
  /**
   * Método msgPalpites()
   * @param: palpites: vetor contendo os palpites do jogador com 
   * "0" coluna do meio, "1" coluna 1" e "2" coluna 2.
   * @return: texto contendo os palpites formatados.
   * Funcionalidade: monta mensagem contendo os resultados dos palpites para 
   * um apostador.
   */ 
  public static String msgPalpites(int [] palpites){
	  ...
	  for(cont = 0; cont < Apostador.MAX_JOGOS; cont++){
              if (palpites[cont] == 1) msg = msg + "Jogo "+(cont+1)+": coluna 1\n";
              else if (palpites[cont] == 0) msg = msg + "Jogo "+(cont+1)+": coluna do meio\n";
              else msg = msg + "Jogo "+(cont+1)+": coluna 2\n";
          }
          return msg;
  }
   ...
   * @param: lista de parâmetros inexistente.
   * @return: nenhum valor de retorno.
   * Funcionalidade: apresentar todos os dados das Apostadors pré-cadastradas no banco de dados
   *               da empresa de “Recursos Humanos e Relacionamentos Cruzeiro do Sul S.A”
   *               utilizando uma interface para os dados de uma pessoa.
   */ 
   ...
   * Funcionalidade: apresentar o menu de 13 opções ao usuário, obter a 
   * 			     opção selecionada, validar se opção existe e retornar
   *                 o valor validado.
   */   
   ...
   * @param: lista de parâmetros inexistente.
   * @return: nenhum valor de retorno.
   * Funcionalidade: cadastra os dados de um apostador no banco. 
   *                 Necessário ler os dados (campos) de cada apostador, UM A UM, por meio de
   *                 JOptionPane.showInputDialog para cada campo
   *                 e inserir esses dados no banco da forma:
   *                     bancoApostadores[++totalApostadoresBanco] = new Apostador();
  ...
  public static void cadastrarApostasDeJogos(){
    /*JOptionPane.showMessageDialog(null, 
			  "Lógica do Método em Java\n\n"
                                  ...
                                  + "deve ser desenvolvido.");  
      */
      int[] palpiteJogadores = new int [10];
      for (int i = 0; i < totalApostadoresBanco; i++) {
          for (int j = 0; j < 10; j++) {

              int valor = Integer.parseInt(JOptionPane.showInputDialog(null, "Esolha as apostas dos jogo "+(j + 1) +"\nValoresVálidos são apenas"
                      + "\n1 - 0 - 2",
                      "escolha o valor", JOptionPane.QUESTION_MESSAGE));
              
              if(valor != 1 && valor != 0 && valor != 2){
                  JOptionPane.showMessageDialog(null, "Válidos apenas os números 1 - 0 - 2\n TENTE NOVAMENTE!","VALOR ERRADO", JOptionPane.INFORMATION_MESSAGE);
                  j = j-1;
                  
              }else{
                  palpiteJogadores[j] = valor;
              }
          }
      }
  }
   ...
   * Funcionalidade: verifica se o usuário deseja realmente finalizar o sistema.
   ...
   *       apresentar o menu de 13 opções e chamar os módulos correspondentes a 
   *       cada uma.
   */ 
  public static void main(String [] args){
	  ...
	  while (opcao != TERMINA_SISTEMA){
	        ...
	        switch( menuOpcoes()){
	            ...
	            case 1: fornecerValorXDaLoteca(); break;
	            
	            // 2 - cadastrar resultado do concurso
	            case 2: cadastrarResultadoConcurso(); break;
	            
                    // 3 - Cadastrar Apostas de Jogos 
	            case 3: cadastrarApostasDeJogos(); break;
	            
	            // 4 - Apresentar Apostadores (implementado)
	            case 4: apresentarApostadores(); break;
	            
	            // 5 - Apresentar Valores (Total e Prêmios)	
	            case 5: apresentarValoresTotalEPremios(); break;
	            
	            // 6 - Apresentar Acertos
	            case 6: apresentarAcertos();
	                  break;
	                  
	            // 7 - Apresentar Jogos Zerados
	            case 7: apresentarJogosZerados();
	                  break;
	        
 	            // 8 - Apresentar Ganhadores 10 acertos
	            case 8: apresentarGanhadores10Acertos(); break;
	            
                    // 9 - Apresentar Ganhadores 9 acertos 
	            case 9: apresentarGanhadores9Acertos(); break;
	            
	            // 10 - Apresentar Percentual Sexo Biológico
	            case 10: apresentarPercentualSexoBiologico(); break;
	            
	            // 11 - Apresentar Percentual Traço Marcante
	            case 11: apresentarPercentualTracoMarcante(); break;
	            
	            // 12 - Apresentar Traço Ganhadores 10 acertos.
	            case 12: apresentarTracoGanhadores10Acertos();
	                  break;
	                  
	            // 13 - Limpar Resultados do Concurso.
	            case 13: limparResultadosDoConcurso();
	                  break;

                    // 14 - verifica se o usuário deseja realmente finalizar o sistema.
	            //      Se sim, retorna a constante TERMINA_SISTEMA, caso contrário,
	            //      retorna a constante NAO_TERMINA_SISTEMA. (Implementado)
	  	    case 14: opcao = verificarTerminoDoSistema();
	  	                  break;
	       }
	  }
	  JOptionPane.showMessageDialog(null, 
